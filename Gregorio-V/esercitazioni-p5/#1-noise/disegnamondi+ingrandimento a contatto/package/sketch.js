...
class Ball {
  ...
  update () {
      ...
      this.x = noise(this.randomX + millis() * 0.0001 * xdirection) * width;
      this.y = noise(this.randomY + millis() * 0.0001 * ydirection) * height;
  ...
  draw () {
    ...
    r = random(3); // r is a random number between 0 - 255
    g = random(3); // g is a random number betwen 100 - 200
    b = random(3); // b is a random number between 0 - 100
...
function draw() {
  fill(255,255,255,3);  // mette lo sfondo con opacità bassa fa l'effetto coda
